...
public class Activity_detail extends AppCompatActivity {
    ...
    Adapter_seminaInfo adapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.semi_detail);

        if (requestQueue == null) {
            requestQueue = Volley.newRequestQueue(getApplicationContext());
        }

        recyclerView = findViewById(R.id.semiDetailRecycler);

        LinearLayoutManager layoutManager = new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false);
        recyclerView.setLayoutManager(layoutManager);

        adapter = new Adapter_seminaInfo();
    ...
    public void makeRequest() {
        ...
        String url = "http://192.168.35.53:4525/test/seminaMobileController?semiCode=1100024";
    ...
    public void processResponse(String response) {
        Gson gson = new Gson();
        SeminarList semilist = gson.fromJson(response, SeminarList.class);


        for (int i = 0; i < semilist.getSeminar_Result().size(); i++) {
            // 해당 ArrayList객체에서 단위 데이터를 가져오서..
            SeminarVO semi = semilist.getSeminar_Result().get(i);
            // adapter에 할당 처리..
            adapter.addItem(semi);
        }
