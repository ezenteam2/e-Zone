...
public class MainActivity extends AppCompatActivity {
    ...
    public void processResponse(String response) {
        ...
        println("영화정보의 수 : " + movieList.getBoxOfficeResult().getDailyBoxOfficeList().size());

        for (int i = 0; i < movieList.getBoxOfficeResult().getDailyBoxOfficeList().size(); i++) {
            // 해당 ArrayList객체에서 단위 데이터를 가져오서..
            Movie movie = movieList.getBoxOfficeResult().getDailyBoxOfficeList().get(i);
            // adapter에 할당 처리..
