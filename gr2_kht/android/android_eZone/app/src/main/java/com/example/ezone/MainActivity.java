...
public class MainActivity extends AppCompatActivity {
    ...
    public void processResponse(String response) {
        ...
        println("공지사항 수 : " + noticeList.getNotice_result().getNotice_LIst().size());

        for (int i = 0; i < noticeList.getNotice_result().getNotice_LIst().size(); i++) {
            // 해당 ArrayList객체에서 단위 데이터를 가져오서..
            Notice_VO notice = noticeList.getNotice_result().getNotice_LIst().get(i);
            // adapter에 할당 처리..
